...
import { betterType, getObjectValue } from '@martingale/utils';
...
class PageSchema {
  ...
  getHandler(from) {
    ...
    return this.handlers.reduce((special, key) => {
      ...
      return from && typeof from[key] !== 'undefined'
        ? this[key].bind(this)
        : false;
    }, false);
  }
  ...
  createPropMap({ from, key }) {
    ...
    if (type === 'array') {
      const mapItemFuncs = from.map((item, index) =>
        this.createPropMap({ from: item, key: index })
      );
  ...
  $mapper(src) {
    ...
    return () => (props) =>
      this.mapValues(src, Object.assign({}, { props }, this.params));
  }

  getPropMap(rawProps) {
    const mappedProps =
      rawProps &&
      Object.keys(rawProps).map((key) => {
  ...
  $component(src, srcProps, srcChildren) {
    ...
    const { $type, children: compChildren, props: rawProps } = src;
    ...
    return (props) => {
      ...
      if (mapped && mapped.children) {
        return (
          <Type key={key} {...mapped}>
            {this.render({ layout: mapped.children })}
          </Type>
        );
      }
...
export { PageSchema, pageSchemaToReact };
